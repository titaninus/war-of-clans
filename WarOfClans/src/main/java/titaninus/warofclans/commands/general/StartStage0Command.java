...
public class StartStage0Command implements WOCCommandManager.RegistrableCommand {
    @Override
    public void Initialize(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess registryAccess, CommandManager.RegistrationEnvironment environment) {
        dispatcher
                .register(literal("startStage0")
                        .requires(source -> source.hasPermissionLevel(4))
                        .executes(StartStage0Command::execute));
    }

    private static int execute(CommandContext<ServerCommandSource> context) {
        GameMaster.Instance().StartStage0();

        context.getSource().sendMessage(Text.literal("Stage0 started!"));
        return 1;
    }
}
